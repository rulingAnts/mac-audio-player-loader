...
- **USB devices and SD cards.** Targets are external USB disks *plus* cards in the Mac's built-in SD slot (which enumerates as internal — admitted only as removable media on a card-reader protocol, so an internal SSD/HDD can never qualify). Every safeguard applies to both alike, and disks are cleanly unmounted before erasing so macOS doesn't raise spurious "Disk Not Ejected Properly" notifications.
- **Player-aware content check before anything is erased.** You pick your player model from the documented families; the loader then validates your folder against *that player's* rules — playable file types, the folder depth it reads audio from, documented naming rules — plus generic checks (loose root files, mixed numbered/un-numbered names). Encrypted-only players (MegaVoice Envision, Davar/Kivah Audibible) are refused with an explanation.
- **Optional simple numeric names.** A per-run choice to write every folder and audio file as `001`, `002`, … in play order (files keep their extension; numbering restarts inside each folder; the source is never touched) — for players that mishandle long or unusual names.
- **Write-order preview, on demand.** The final erase confirmation states the rule — files are written in filename order (add `001`, `002`… prefixes to change it) — and its **Preview order…** button opens a collapsible folder tree of exactly what will be written, original names alongside any renamed device names (a single "write manifest" drives both the preview and the copy, so they always match). No extra dialog interrupts repeat batches of the same content.
- **Problems marked in place.** Every content-check finding is itemized by exact path and written on the item itself in the preview tree (wrong file type, wrong folder depth, loose root file, empty folder, name problems); parent folders of a flagged item are marked and pre-expanded, and the preview opens automatically whenever anything is flagged.
