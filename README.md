...
- **Write-order preview.** States the rule up front — files are written in filename order (add `001`, `002`… prefixes to change it) — and offers a full, scrollable browser preview of the exact order (a single "write manifest" drives both the preview and the copy, so they always match). The rule is repeated in the erase-confirmation dialog.
